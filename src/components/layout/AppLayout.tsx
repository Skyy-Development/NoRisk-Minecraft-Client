...
export function AppLayout({
  ...
  const renderBackgroundEffect = () => {
    switch (currentEffect) {
      ...
      case BACKGROUND_EFFECTS.STARFIELD:
        return (
          <StarfieldEffect
            opacity={qualityParams.opacity * 1.5}
            speed={qualityParams.speed * 0.8}
            starCount={qualityParams.particleCount * 4}
          />
        );
      case BACKGROUND_EFFECTS.FIREFLIES:
        return (
          <FirefliesEffect
            opacity={qualityParams.opacity * 1.5}
            speed={qualityParams.speed}
  ...
  return (
    <div
      ref={launcherRef}
      className="h-screen w-full bg-black/50 backdrop-blur-lg border-2 overflow-hidden relative flex shadow-[0_0_25px_rgba(0,0,0,0.4)] rounded-xl"      style={{
...
function BorderGlowEffects({ accentColor }: { accentColor: string }) {
  return (
    <>
      ...
      ></div>
      <div
        className="absolute top-0 bottom-0 left-0 w-[2px] rounded-l-xl"
        style={{
          background: `linear-gradient(to bottom, transparent, ${accentColor}70, transparent)`,
        }}
      ></div>
      <div
        className="absolute top-0 bottom-0 right-0 w-[2px] rounded-r-xl"
        style={{
          background: `linear-gradient(to bottom, transparent, ${accentColor}70, transparent)`,
        }}
      ></div>
